...
void app_main(void)
{
    ...
    pm_device_state_set_health(&device_state, security_status == ESP_OK ? PM_DEVICE_HEALTH_OK : PM_DEVICE_HEALTH_WARN);

    ESP_LOGI(TAG, "Plant-Management firmware scaffold started");
    ESP_LOGI(TAG, "Device role: %s", config.device_role);
    ESP_LOGI(TAG, "MQTT transport required TLS: %s", config.require_tls ? "true" : "false");
